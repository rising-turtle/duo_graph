...
bool Node::getRelativeTransformationTo_ICP_code(const Node* target_node,
                                                ...
                                                const Eigen::Matrix4f& initial_transformation) const
{
...
#ifdef USE_ICP_CODE
void Node::Eigen2GICP(const Eigen::Matrix4f& m, dgc_transform_t g_m) const
{
  for(int i=0;i<4; i++)
    for(int j=0;j<4; j++)
      g_m[i][j] = m(i,j);
...
void Node::GICP2Eigen(const dgc_transform_t g_m, Eigen::Matrix4f& m) const
{
  for(int i=0;i<4; i++)
    for(int j=0;j<4; j++)
      m(i,j) = g_m[i][j];
            ...
            || found_transformation)
        {
            ...
            if(!containsNaN(mr_icp.final_trafo)) 
            {
                ...
                { //This signals a valid result:
                    ...
                    //liu change inlier points
                    mr.inlier_points = mr_icp.inlier_points;
                    mr.outlier_points = mr_icp.outlier_points;
                    mr.occluded_points = mr_icp.occluded_points;
                    mr.all_points = mr_icp.all_points;
                    return true;
                }
            }
        }
  ...
  try{
    ...
    if (mr.all_matches.size() < (unsigned int) ps->get<int>("min_matches")){
        ...
        if(getRelativeTransformationTo(older_node,&mr.all_matches, mr.ransac_trafo, mr.rmse, mr.inlier_matches))
        {
          ...
          //liu: the algorithm easy to lose if we set observation_threshold as positive, so always set valid_tf to true
          bool valid_tf = true;
