...
CSubmap<NODE>::CSubmap():
    ...
    m_pOctoTree(new ColorOctreeImpl(gl_octomap_voxel_size)),
    // m_submap_store_id(0)
    id_(0),
...
void CSubmap<NODE>::addNode(NODE* node)
{
    ...
    pass_through.setFilterLimits(gl_pass_through_min_z, gl_pass_through_max_z);
...
void CSubmap<NODE>::dump2File(string fname)
{
    ...
    // save all the
    stringstream ss;
    ss<<gl_submap_saved_path<<"/"<<fn<<".map";
    ...
    // 7.1 ".pcd"
    {
        // save all the
        stringstream ss;
        ss<<gl_submap_saved_path<<"/"<<fn<<".pcd";
        if(m_bUseOctomap)
        {
            pcl::io::savePCDFile(ss.str(),*pc_col);
        }else
        {
              // ROS_ERROR("Submap.hpp Voxel_grid enabled!");
              pcl::VoxelGrid<pcl::PointXYZRGB> voxel_grid;
              voxel_grid.setInputCloud (pc_col);
              float leaf_size = gl_octomap_voxel_size;
