...
MatchingResult CSubmapNode::matchNodePair(CSubmapNode* older_node)
{
    ...
    // ROS_ERROR("CSubmapNode.cpp: try to match Node %d with Node %d", older_node->id_, this->id_);
    try{
        ...
        {//All good for feature based transformation estimation
            ...
            {   
                // ROS_ERROR("SubmapNode.cpp: failed to match with inliers: %d", mr.inlier_matches.size());
                mr.inlier_matches.clear();
            }
        } 
        if(found_transformation) 
        {
            // ROS_ERROR("SubmapNode.cpp succeed to match between %d and %d with inliers: %d", older_node->id_, this->id_, mr.inlier_matches.size());
